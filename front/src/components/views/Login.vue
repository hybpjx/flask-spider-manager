<template>
  ...
  <!--   注册登陆     -->
  <div class="login-wrap box container">
      ...
      login(data).then(res => {
        if (res.status === 200) {
          if (res.data.status === 200) {
            let referrer = sessionStorage.setItem('referrer', '#sd1^123sd');
            this.$router.push('/index');

            this.$notify({type: 'success', message: '恭喜，登录成功！'});
            // this.$router.push("/index");
          } else {
            this.$notify({type: 'error', message: '对不起，请你重新登陆！'});
          }
